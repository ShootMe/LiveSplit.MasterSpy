...
namespace LiveSplit.MasterSpy {
	...
	public class ProgramPointer {
		...
		public string ReadString(Process program, int length, params int[] offsets) {
			GetPointer(program);
			return program.ReadString(Pointer, length, offsets);
		}
		...
		private IntPtr GetVersionedFunctionPointer(Process program) {
			if (signatures != null) {
				...
				searcher.MemoryFilter = delegate (MemInfo info) {
					return info.State == 0x1000 && info.Type == 0x20000 && info.Protect == 0x4;
				};
